...
import { cars as allCars } from "./data/carsData"; 
...
export default function Home() {
  ...
  // Marca cuáles quieres mostrar como "destacados" en la landing. Sería bueno escribir una funcion que lo haga automáticamente según algún criterio.
  ...
  //  Uso de SLUGS en vez de ids numéricos a partir de carsData.ts.
  const vehicles = allCars.map((c) => ({
    id: c.id,
    brandId: c.brandId,                 
    brand: c.brand,           
    name: c.model,            
  ...
  // Filtro por selección (featured / all / marca)
  const filteredVehicles =
  selectedBrand === "featured"
    ? vehicles.filter((v) => v.featured)
    : selectedBrand === "all"
    ? vehicles
    : vehicles.filter((v) => v.brandId === selectedBrand);
    ...
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-50">
        <div className="animate-pulse-neon">
          <img src="/Imagenes/Logo.png" alt="Green Motion" className="w-32 h-32 object-contain drop-shadow-2xl" />
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black animate-reveal">
      ...
      <nav className="fixed top-0 w-full bg-transparent backdrop-blur-md z-50 border-b border-[#00764e]">
        <div className="grid grid-cols-3 items-center h-20 w-full">
          ...
          <div className="flex justify-end">
            <button className="modern-button text-white px-6 py-3 rounded-lg font-semibold modern-glow">Cotizar</button>
          </div>
        </div>
      </nav>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <Image src="/Imagenes/Fondo.JPG" alt="Fondo Green Motion" fill className="object-cover" priority quality={85} sizes="100vw" />
          <div className="absolute inset-0 bg-black/75" />
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/50" />
        <div className="relative z-10 text-center max-w-4xl mx-auto px-4">
          <h1 className="text-6xl md:text-8xl font-black mb-6 text-white">El Futuro <span className="block bg-gradient-to-r from-[#00764e] to-[#47ba41] bg-clip-text text-transparent">es Eléctrico</span></h1>
          <p className="text-xl md:text-2xl text-gray-200 mb-8 max-w-2xl mx-auto font-light">
            Descubre las marcas más exclusivas de vehículos eléctricos. <span className="text-[#47ba41] font-medium"> Lujo, potencia y sostenibilidad</span> en un solo lugar.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button className="modern-button text-white px-8 py-4 rounded-lg font-bold text-lg modern-glow">Explorar Modelos</button>
            <button className="modern-outline px-8 py-4 rounded-lg font-semibold text-lg">Agendar Test Drive</button>
          </div>
        </div>
      ...
      <section className="py-12 bg-black/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="h-1 bg-gradient-to-r from-[#00764e] via-[#47ba41] to-[#289b8f] mt-8" />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {filteredVehicles.length > maxVisible && (
              <button onClick={prevSlide} className="absolute -left-6 top-1/2 -translate-y-1/2 z-10 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white p-3 rounded-full border border-white/20 hover:border-white/40">
            ...
            {filteredVehicles.length > maxVisible && (
              <button onClick={nextSlide} className="absolute -right-6 top-1/2 -translate-y-1/2 z-10 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white p-3 rounded-full border border-white/20 hover:border-white/40">
            ...
            <div className="overflow-hidden">
              ...
              >
                {filteredVehicles.map((v, index) => (
                  <div
                    key={v.id}
                    className="bg-black/80 backdrop-blur-sm rounded-xl overflow-hidden hover:bg-black/90 transition-all duration-300 group shadow-lg hover:shadow-xl hover:shadow-[#47ba41]/20 flex flex-col flex-shrink-0"
                    style={{ width: `calc((100% - ${(maxVisible - 1) * 32}px) / ${maxVisible})` }}
                  >
                    <div className="relative h-72 overflow-hidden">
                      ...
                      <div className="absolute top-4 left-4 z-20">
                        <span className="bg-[#00764e] text-white px-4 py-2 rounded-full text-sm font-semibold">{v.brand}</span>
                      </div>
              ...
              <div className="flex justify-center mt-6 space-x-2">
                {Array.from({ length: Math.max(1, filteredVehicles.length - maxVisible + 1) }, (_, i) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-all duration-300 ${currentIndex === i ? "bg-[#47ba41]" : "bg-gray-600 hover:bg-gray-400"}`}
                  />
                ))}
              </div>
            )}
